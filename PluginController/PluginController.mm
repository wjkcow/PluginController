...
- (id) init{
    ...
    if (self) {
        ...
        _vimrcFile = [[NSArray alloc] init];
        _snippets  = [[NSArray alloc] init];
       // _mapFile = [[NSArray alloc] init];
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath: cfgPath]) {
            NSLog(@"%@ found", cfgPath);
            _vimrcFile = [PCfileMag getFromPath: [homeDir stringByAppendingString:vimrcPathEnd]];
            [self loadPlist];
            _snippets = [PCSnippet getAllSnippet];
            [self savePlist];
...
- (void) savePlist{
    ...
    NSData *plistData = [NSPropertyListSerialization dataFromPropertyList: plistDict
                                                                   format:NSPropertyListXMLFormat_v1_0 errorDescription:&errorMsg];
...
- (void) initVimrc{
    ...
    NSString *originVimrc = [NSString stringWithContentsOfFile:vimrcPath encoding:NSASCIIStringEncoding error:nil];
    NSMutableDictionary *dict = [NSMutableDictionary new];
    [dict setObject:@"\"content" forKey:@"tag"];
    [dict setObject:@"\"content" forKey:@"tag1"];

    if(originVimrc != NULL){
        originVimrc = [NSString stringWithFormat:@"\n\"Dont't add before this\n%@", originVimrc];
    } else{
        originVimrc = [NSString stringWithFormat:@"\n\"Dont't add before this\n"];
    }
    _vimrcFile = @[dict, originVimrc];
    [PCfileMag writeToPath:vimrcPath fromArray: _vimrcFile];
}
//new
- (void) snippet{
...
- (void) test{
    NSMutableArray *dict = [PCSnippet getAllSnippet];
    [PCSnippet writeAllSnippet:dict];
}
...
//new
- (void )installSnipMate{
    NSDictionary *items = [self search:@"snipMate"];
    NSNumber *id_pc = [[NSNumber alloc] initWithInt:1084353];
    ...
    [_vimrcFile[0] setObject:@":filetype plugin on" forKey:@"snipMateInstall"];
    [self saveVimrc];
}
- (void) installPathogen{
    ...
    NSData *pathogenVim = [[NSData alloc] initWithContentsOfURL:pathogenUrl];
    [[NSFileManager defaultManager] createFileAtPath:pathogenPath contents:pathogenVim attributes:nil];
    [_vimrcFile[0] setObject:@":call pathogen#infect()" forKey:@"pathogenInstall"];
    [self saveVimrc];
}
@end
